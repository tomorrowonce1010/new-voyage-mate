...
export default function ChatMain({ current, msgs, input, setInput, onSend, myId, friendList }) {
    ...
    <div className="chat-main">
      ...
      <div className="chat-body" ref={bodyRef}>
        {current && msgs.map((msg, i) => {
          ...
          // 渲染消息时，统一用msg.userId === myId判断isMe，控制气泡左右和样式
            ...
            <div key={msg.messageId || i} style={{display:'flex', flexDirection:'column', alignItems: isMe ? 'flex-end' : 'flex-start', marginBottom: '16px'}}>
              <div style={{display:'flex', flexDirection: isMe ? 'row-reverse' : 'row', alignItems:'flex-end', width:'100%'}}>
                <div style={{maxWidth:'70%', display:'flex', flexDirection:'column', alignItems: isMe ? 'flex-end' : 'flex-start'}}>
                  ...
                  <div className="msg-bubble" style={{background: isMe ? '#e3f0ff' : '#fff', color:'#222', borderRadius: '12px', padding:'8px 14px', fontSize:'1.05em', boxShadow:'0 1px 4px #eee', textAlign:'left', wordBreak:'break-all', minWidth: '36px'}}>
                    {msg.content}
                  </div>
